...
pub use super::commands::{AnyCommand, Command, State};
...
impl Purser {
    ...
    pub async fn send(&mut self, resolver: &mut Resolver, recipient: &Name, request: ChandlerRequest) -> Result<ChandlerResponse, Error> {
        ...
        let url = resolver.lookup(recipient, &now(), "air_url").await.map_err(Error::mr)?;
    ...
    pub async fn send_batch(&mut self, resolver: &mut Resolver, requests: Vec<Request>) -> Vec<Response> {
        ...
        let mut results: BTreeMap<Name, Result<Vec<ChandlerResponse>, Error>> = BTreeMap::new();
        for (recipient, batch) in batches {
            results.insert(*recipient, self.send(resolver, recipient, ChandlerRequest::batch(batch)).await.and_then(|b| b.batch()));
        }
        requests.into_iter().rev().map(|(_, es)| es.into_iter().map(|e| results.get_mut(&e).unwrap().as_mut().map(|r| r.pop().unwrap()).map_err(|e| e.clone())).collect()).collect()
    }
}

pub type Compiler<Output> = commands::Compiler<Request, Purser, Output>;
pub type Context = commands::Context<Request>;
pub type Mapped<I, O> = commands::Mapped<Request, I, O>;
pub type Request = (ChandlerRequest, Vec<Name>);

type Response = Vec<Result<ChandlerResponse, Error>>;
impl commands::Request for Request {type Response = Response;}

impl commands::Handler<Request> for Purser {
...
impl<I: RawRequest + Any + Send> Command<Request> for (I, Name) {
    ...
    async fn run(self, ctx: Context) -> Self::Output {
        vec![(self.0, vec![self.1])].run(ctx).await.remove(0).remove(0)
    }
}
...
impl<I: RawRequest + Any + Send> Command<Request> for (I, Vec<Name>) {
    ...
    async fn run(self, mut ctx: Context) -> Self::Output {
        ctx.send(vec![(self.0.into(), self.1)]).await.remove(0).into_iter().map(|r| r.and_then(I::from)).collect()
    }
}
